...
| Device | Purpose & OSI Layer | Deployment | Use Cases | Key Fields to Investigate | Popular Vendors |
|--------|---------------------|-----------------------------|------------|--------------------------|-----------------|
| 🔥 **Firewall** | - Monitors and filters network traffic based on security rules.<br>- Operates at OSI Layer 3 (Network) and Layer 4 (Transport).<br><br>*Analogy:* **Like a security guard at a building entrance, checking who can enter or leave.** | - Hardware or software.<br>- Deployed inline with network traffic. | - Detecting too many denied/allowed connections<br>- Blocking traffic from suspicious countries<br>- Monitoring firewall configuration changes<br>- Investigating excessive VPN access failures<br>- Identifying communication with known bad IPs | - Source<br>- Destination<br>- Port<br>- Action<br>- Protocol<br>- Rule/Policy ID<br>- Timestamp<br>- Interface | - Palo Alto Networks<br>- Fortinet (FortiGate)<br>- Cisco Firepower<br>- Check Point<br>- Juniper SRX |
| 🛡️ **IDS/IPS** | - IDS detects suspicious activity and alerts.<br>- IPS blocks/prevents threats in real time.<br>- Operates at Layer 3 and 4.<br><br>*Analogy:* **Like a security alarm system that detects break-ins (IDS) or automatically locks doors to stop intruders (IPS).** | - Hardware or software.<br>- IDS is passive.<br>- IPS is deployed inline. | - Detecting high-severity attacks on multiple machines<br>- Identifying repeated attacks from a single public IP<br>- Alerting on new/unknown attack patterns<br>- Monitoring for lateral movement | - Source<br>- Destination<br>- Port<br>- Signature/Alert Type<br>- Severity<br>- Protocol<br>- Timestamp<br>- Action | - Palo Alto Networks<br>- Cisco Secure IPS<br>- Fortinet<br>- Trend Micro TippingPoint<br>- Snort<br>- Suricata |
| 🕸️ **WAF** | - Protects web applications by filtering HTTP/HTTPS traffic.<br>- Blocks web attacks.<br>- Operates at Layer 7 (Application).<br><br>*Analogy:* **Like a bouncer at a club, only letting in guests who follow the rules and blocking troublemakers.** | - Cloud service or on-premises appliance.<br>- Deployed inline. | - Blocking automated attacks (bots, scanners)<br>- Preventing exploitation of web vulnerabilities<br>- Monitoring abnormal HTTP requests<br>- Protecting sensitive APIs | - Client<br>- HTTP Method<br>- URI/URL<br>- Rule/Attack Type<br>- Status Code<br>- User Agent<br>- Timestamp | - AWS WAF<br>- Cloudflare WAF<br>- Imperva<br>- F5 BIG-IP<br>- Akamai Kona Site Defender |
| 🦠 **Antivirus/EDR** | - Scans for and removes malware.<br>- EDR detects, investigates, and responds to endpoint threats.<br>- Operates on endpoints.<br><br>*Analogy:* **Like a doctor who checks for and treats infections on computers.** | - Software.<br>- Supports real-time or on-demand scanning. | - Responding to malware outbreaks<br>- Investigating multiple detections on one endpoint<br>- Detecting AV/EDR tampering<br>- Analyzing suspicious endpoint behavior | - Hostname/Endpoint<br>- Username<br>- Malware/Threat Name<br>- File Path<br>- Action Taken<br>- Timestamp<br>- Process/Parent Process | - CrowdStrike Falcon<br>- SentinelOne<br>- Microsoft Defender for Endpoint<br>- Trellix (formerly McAfee)<br>- Sophos Intercept X |
| 🧑‍💻 **Proxy** | - Intermediary filtering web traffic (HTTP/HTTPS).<br>- Enforces content policies and anonymizes users.<br>- Operates at Layer 7.<br><br>*Analogy:* **Like a librarian who checks what books you borrow and makes sure you follow library rules.** | - Hardware or software.<br>- Deployed inline or as a forward proxy. | - Detecting excessive HTTP requests<br>- Blocking access to malicious/unauthorized sites<br>- Monitoring bypass attempts<br>- Enforcing acceptable use policies | - User<br>- Client<br>- URL/Domain<br>- HTTP Method<br>- Category<br>- Action<br>- Timestamp | - Zscaler Internet Access<br>- Cisco Umbrella<br>- Forcepoint<br>- Blue Coat (Symantec ProxySG)<br>- Squid Proxy |
| 🗄️ **DLP** | - Monitors and controls movement of sensitive data.<br>- Prevents unauthorized sharing or leakage.<br>- Operates at Layer 7.<br><br>*Analogy:* **Like a customs officer checking luggage to prevent smuggling of valuables.** | - Software or appliance.<br>- Deployed inline or as an endpoint agent. | - Detecting attempts to send sensitive data via email<br>- Blocking uploads to cloud storage<br>- Alerting on large data transfers<br>- Preventing data exfiltration | - User<br>- Endpoint/Device<br>- Data Type<br>- Action<br>- Destination<br>- File Name/Path<br>- Timestamp | - Microsoft Purview (DLP)<br>- Forcepoint DLP<br>- Symantec DLP<br>- McAfee DLP<br>- Digital Guardian |
| ✉️ **Email Gateway** | - Filters inbound and outbound emails.<br>- Blocks spam, phishing, and malware.<br>- Operates at Layer 7.<br><br>*Analogy:* **Like a mailroom clerk who checks all incoming and outgoing mail for suspicious packages.** | - Cloud or on-premises appliance.<br>- Deployed inline with email flow. | - Detecting/blocking phishing or spam<br>- Identifying malware in attachments/links<br>- Investigating suspicious link clicks<br>- Monitoring large email volumes<br>- Flagging suspicious content/subjects | - Date/Time<br>- Name<br>- Message<br>- Device Vendor<br>- Device Product<br>- Email Subject<br>- Email Sender<br>- Email Recipient<br>- Sender Server<br>- Sender Hello String<br>- Delivery Location<br>- Status<br>- Links/URLs<br>- Attachment Hash | - Microsoft Defender for Office 365<br>- Proofpoint<br>- Mimecast<br>- Cisco Secure Email (Ironport)<br>- Barracuda |
| 🖥️ **SIEM** | - Aggregates and analyzes logs from multiple devices.<br>- Provides real-time monitoring and alerting.<br>- Operates across all OSI layers.<br><br>*Analogy:* **Like a central command center that collects and analyzes security data from various sources.** | - Software or cloud service.<br>- Deployed centrally. | - Correlating events from different sources<br>- Detecting complex attack patterns<br>- Generating compliance reports<br>- Investigating security incidents | - Log Source<br>- Event Type<br>- Severity<br>- Timestamp<br>- User<br>- Source<br>- Destination<br>- Action | - Splunk<br>- IBM QRadar<br>- ArcSight<br>- LogRhythm<br>- Microsoft Sentinel |
    ...
    <li>
        ...
        <details>
            ...
            <ul>
                ...
                <li><strong>Use Cases:</strong>
                    <ul>
                        <li>Detecting high-severity attacks on multiple machines.</li>
    ...
    <li>
        ...
        <details>
            ...
            <ul>
                ...
                <li>Supports real-time (on-access) and scheduled (on-demand) scanning.</li>
            ...
            <ul>
                ...
                <li><strong>Fields to investigate:</strong>
                    <ul>
                        ...
                        <li><code>sender_server_ip</code>: IP address of the sending server.</li>
